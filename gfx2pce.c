...
int Convert2Pic(char *filebase, unsigned char *buffer,
				int num_tiles, int blank_absent, int colors)
{
	char filename[80];
	int x,y,t,b;
	int i;
...
void ConvertPalette(RGB_color *palette, int *new_palette) {
	...
	for(i=0;i<256;i++) {
		if(palette_rnd) {
			...
			data += ((palette[i].red >> 3) + (temp & rounded))<<3;				//round up if necessary
			rounded = (temp ^ rounded);			//reset rounded down flag after rounding up

			//get blue portion and round it
			temp = (palette[i].blue & 0x01);	//see if this needs rounding
			if(palette[i].blue == 63) { //if value == 63, then we can't round up
				temp = 0;
				rounded = 1;
			}
			data += (palette[i].blue >> 3) + (temp & rounded);				//round up if necessary
			rounded = (temp ^ rounded);			//reset rounded down flag after rounding up
			...
			// pce color format G3:R3:B3
			data  = (palette[i].green >> 3)<<6;
			data += (palette[i].red >> 3)<<3;
			data += (palette[i].blue >> 3);
...
int main(int argc, char **arg) {
	...
	for(i=1;i<argc;i++)	{
		if(arg[i][0]=='-') {
			...
			else if(arg[i][1]=='m') //map options
			{
				...
				else if(arg[i][2]=='n') //offset for tilebase
				{
					tilebase = atoi(&arg[i][3]);
				}
			...
			else if(arg[i][1]=='p') //palette options
			{
				...
				{
					colors=atoi(&arg[i][3]);
					if( (colors !=16) && (colors !=32) &&  (colors != 128) && (colors != 256) )
					{
						PrintOptions(arg[i]);
						return 1;
					}
	...
	if(!Convert2Pic(filebase, buffer, num_tiles, blank_absent, colors))
	{
		if(screen)
			free(tilemap);
		free(buffer);
